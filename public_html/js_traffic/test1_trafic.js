...
/////СЕКЦИЯ 0
	...
	//ПРАВО
		if(section.index == 4 && direction == 'right'){
			anime({
					  targets: '.sec4_slide_0 .sl4_0',
					  keyframes: [
						{translateX: 10},
						{translateX: -5},
						{translateX: 0}
		...
		// ЛЕВО
               if(section.index == 4 && direction == 'left'){
			anime({
					  targets: '.sec4_slide_0 .sl4_0',
					  keyframes: [
						{translateX: -10},
	...
	onLeave: function(origin, destination, direction){
		...
		//после покидания раздела 4
		if(origin.index == 3 && direction =='down'){
			//alert("Going to section 3!");
			
		geometry.vertices.forEach( function(particle, index){
						let tl = new TimelineMax();
						tl.to(particle,1,{x:gallery[4][index][0],y:gallery[4][index][1]});
					})	
